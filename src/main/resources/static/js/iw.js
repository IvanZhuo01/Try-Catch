...
const ws = {
    ...
    receive: (text) => {
        console.log(text);
        if (JSON.parse(text.body).to !== "sin receptor") {
    ...
    subscribe: (sub) => {
        try {
            ws.stompClient.subscribe(sub,
                (m) => ws.receive(m)); // fails if non-json received!
...
document.addEventListener("DOMContentLoaded", () => {
    if (config.socketUrl) {
        ...
        let p = document.querySelector("#nav-unread");
        let notifInbox = document.querySelector("#notiInbox")
        let modal = document.querySelector(".modal-notificaciones");
        if (p) {
            go(`${config.rootUrl}/user/unread`, "GET").then(d => p.textContent = d.unread);
        }
        //MODIFICAR PARA ADMIN, con get
        if (notifInbox) {
            go(`${config.rootUrl}/user/received`, "GET").then(msgs => {
                console.log("LISTA NOTIFICACIONES")
                if (msgs.length !== 0) {
                    document.querySelector("#emptyInbox").classList.add("d-none")
                }
                msgs.forEach(element => {
                    console.log("Iw", element)
                    notifInbox.insertAdjacentHTML("beforeend", renderInboxNotification(element)) //Recibe un json sin parsear
                });
            });
        }
